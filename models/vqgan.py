...
class VQGAN(nn.Module):
    def __init__(self, H):
        ...
        self.disc = Discriminator(H.n_channels, H.ndf, n_layers=H.disc_layers)
    ...
    def train_iter(self, x, step):
        ...
        d_weight = calculate_adaptive_weight(
            nll_loss, g_loss, last_layer, self.disc_weight_max
        )
        ...
        stats["latent_ids"] = (
            quant_stats["min_encoding_indices"].squeeze(1).reshape(x.shape[0], -1)
        )
        ...
        if step > self.disc_start_step:
            ...
            logits_fake = self.disc(
                x_hat.contiguous().detach()
