...
def main(H, vis):
    ...
    with torch.no_grad():
        latents = model.sample_shape(
            shape[1:], H.batch_size, time_steps=time_steps, step=step
        )
        latents_one_hot = latent_ids_to_onehot(latents, shape, H.codebook_size).cuda()

        q = (
            torch.matmul(latents_one_hot, embedding_weight)
            .view(latents_one_hot.size(0), shape[1], shape[2], H.emb_dim)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        all_images = []
