...
def main(H, vis):
    ...
    annotations_dataset = FFHQImageAttributeFolder(
        "/datasets/FFHQ",
    ...
    annotations_dataloader = torch.utils.data.DataLoader(
        annotations_dataset, batch_size=H.batch_size, shuffle=False
    )

    _, ann = next(iter(annotations_dataloader))
    ...
    for batch_num, image_batch in tqdm(
        ...
        image_batch = image_batch[0][0].cuda()  # remove label and annotation
