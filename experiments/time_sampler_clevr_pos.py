...
def main(H, vis):
    ...
    # print total params in generator + sampler model
    print(
        sum(p.numel() for p in generator.parameters())
        + sum(p.numel() for p in sampler.parameters())
    )

    sampler.n_samples = H.batch_size  # get samples in 5x5 grid
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=H.batch_size, shuffle=False
    )
    ...
    for batch in dataloader:
        ...
        print(f"Time taken for batch of {H.batch_size} images: {time_taken}")
