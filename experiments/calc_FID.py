...
from utils.experiment_utils import (
    generate_images_from_latents,
    generate_samples,
    get_generator_and_embedding_weight,
)
...
def main(H):
    real_dataset, _ = get_datasets(
        H.dataset,
        H.img_size,
        custom_dataset_path=H.custom_dataset_path,
        extra_augs=False,
    ...
    if not already_generated:
        ...
        if not H.latents_path:
            log(
                f"Generating {H.n_samples} samples for {H.dataset} dataset with {H.n_components} components"
            )
            generate_samples(H)
        else:
