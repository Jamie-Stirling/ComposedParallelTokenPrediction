...
def main(H):
    ...
    if not H.real_feats:
        ...
        real_dataset, _ = get_datasets(
            H.dataset, H.img_size, custom_dataset_path=H.custom_dataset_path
        )
        real_dataset = NoClassDataset(
            real_dataset, H.n_samples
        )  # n_images defaults to None
        real_data_loader = torch.utils.data.DataLoader(
            real_dataset, batch_size=H.batch_size
        )
        real_features = get_feats_from_loader(real_data_loader)
        timestamp = int(time.time())
        torch.save(
            real_features, f"_pkl_files/{H.dataset}_real_features_{timestamp}.pkl"
        )
    ...
    if not H.fake_feats:
        ...
        fake_data_loader = torch.utils.data.DataLoader(
            fake_dataset, batch_size=H.batch_size
        )
        log("Generating fake samples features")
        fake_features = get_feats_from_loader(fake_data_loader)
        timestamp = int(time.time())
        torch.save(
            fake_features, f"_pkl_files/{H.dataset}_fake_features_{timestamp}.pkl"
        )
    ...
    metrics = compute_prdc(
        real_features=real_features, fake_features=fake_features, nearest_k=3
    )
    log(metrics)
...
if __name__ == "__main__":
    ...
    if H.load_step > 0:
        log(
            f"Running PRDC calculations for sampler in {H.load_dir} at step {H.load_step}"
        )
        start_training_log(H)
        main(H)
    else:
        raise ValueError(
            "No value provided for --load_step, cannot calculate FID for new model"
        )
