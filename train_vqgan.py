...
def main(H, vis):
    ...
    train_loader, val_loader = get_data_loaders(
        H.dataset, H.img_size, H.batch_size, get_val_dataloader=(H.steps_per_eval != 0)
    )
    ...
    if H.load_step > 0:
        ...
        vqgan, optim, d_optim, ema_vqgan, train_stats = load_vqgan_from_checkpoint(
            H, vqgan, optim, d_optim, ema_vqgan
        )
        ...
        if train_stats is not None:
            ...
            if H.steps_per_eval:
                if H.steps_per_eval == 1:
                    ...
                    eval_start_step = (
                        start_step + H.steps_per_eval - start_step % H.steps_per_eval
                    )
    ...
    log(
        f"total params:{sum(p.numel() for p in vqgan.ae.parameters()) + sum(p.numel() for p in vqgan.disc.parameters())}"
    )
    ...
    for step in range(start_step, H.train_steps):
        ...
        if H.steps_per_eval:
            if step % H.steps_per_eval == 0 and step > 0:
                ...
                steps = [
                    step for step in range(eval_start_step, step + 1, H.steps_per_eval)
                ]
                # vis.line(fids, steps, win='FID',opts=dict(title='FID'))
                vis.line(
                    val_losses, steps, win="val", opts=dict(title="Validation L1 Loss")
                )
        ...
        if step % steps_per_epoch == 0 and step > 0:
            ...
            log(
                f"Codebook size: {H.codebook_size}   Unique Codes Used in Epoch: {len(unique_code_ids)}"
            )
            latent_ids = []
