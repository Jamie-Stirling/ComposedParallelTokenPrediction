...
def generate_images_from_latents(H, all_latents, embedding_weight, generator):
    ...
    for idx, latents in tqdm(list(enumerate(torch.split(all_latents, H.batch_size)))):
        latents_one_hot = latent_ids_to_onehot(
            latents, H.latent_shape, H.codebook_size
        ).cuda()
        q = (
            torch.matmul(latents_one_hot, embedding_weight)
            .view(
        ...
        save_images(
            gen_images.detach().cpu(), "sample", idx, H.log_dir, save_individually=True
        )
            ...
            if H.sample_type == "diffusion":
                if H.dataset == "clevr":
                    ...
                    if n_conditions == 0:
                        ...
                        latents = sampler.sample_compositional_clevr(
                        ...
                        raise NotImplementedError(
                            "Please implement conditional sampling for CLEVR dataset"
                        )
                else:
