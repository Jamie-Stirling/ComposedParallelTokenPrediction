...
class HparamsAbsorbing(HparamsBase):
    def __init__(self, dataset):
        ...
        if self.dataset == "clevr_cond":
            self.total_steps = 256 + 4
            self.sample_steps = 256 + 4
        else:
        ...
        if self.dataset == "churches" or self.dataset == "bedrooms":
            ...
            self.dataset == "clevr"
            or self.dataset == "clevr_pos"
            or self.dataset == "clevr_rel"
        ):
            self.batch_size = 20
            self.bert_n_emb = 512
            self.bert_n_head = 8
            self.bert_n_layers = 24
            self.block_size = 256  # 4 attributes
            self.lr = 1e-4
            self.warmup_iters = 30000
        elif self.dataset == "clevr_cond":
            self.batch_size = 20
            self.bert_n_emb = 512
            self.bert_n_head = 8
            self.bert_n_layers = 24
            self.block_size = 256 + 4
            self.lr = 1e-4
            self.warmup_iters = 30000
        else:
            raise KeyError(
                f"Defaults not defined for multinomial diffusion model on dataset: {self.dataset}"
            )
...
class HparamsAutoregressive(HparamsBase):
    def __init__(self, dataset):
        ...
        if self.dataset == "clevr_pos":
        ...
        elif self.dataset == "churches" or "bedrooms":
            self.batch_size = 20
            self.bert_n_emb = 512
            self.bert_n_head = 8
            self.bert_n_layers = 24
            ...
            raise KeyError(
                f"Defaults not defined for BERT model on dataset: {self.dataset}"
            )
...
def add_sampler_args(parser):
    ...
    parser.add_argument(
        "--loss_type", type=str, choices=["reweighted_elbo", "elbo", "mlm"]
    )
    ...
    parser.add_argument(
        "--sampler", type=str, required=True, choices=["absorbing", "autoregressive"]
    )
