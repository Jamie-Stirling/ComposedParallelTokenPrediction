...
class HparamsVQGAN(HparamsBase):
    def __init__(self, dataset):
        ...
        if self.dataset == "churches" or self.dataset == "bedrooms":
            ...
            self.attn_resolutions = [16]
            self.batch_size = 8
            self.ch_mult = [1, 1, 2, 4]  # [1, 1, 2, 2, 4] for ffhq
            self.codebook_size = 256
            self.disc_layers = 3
            self.disc_weight_max = 0.5
            self.disc_start_step = 1000000  # essentially never
            self.emb_dim = 256
            ...
            raise KeyError(
                f"Defaults not defined for VQGAN model on dataset: {self.dataset}"
            )
...
def add_vqgan_args(parser):
    ...
    parser.add_argument(
        "--gumbel_straight_through", const=True, action="store_const", default=False
    )
    parser.add_argument(
        "--horizontal_flip", const=True, action="store_const", default=False
    )
