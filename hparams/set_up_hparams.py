import argparse
from .defaults.sampler_defaults import (
    HparamsAbsorbing,
    HparamsAutoregressive,
    add_sampler_args,
)
from .defaults.vqgan_defaults import HparamsVQGAN, add_vqgan_args
from .defaults.experiment_defaults import (
    add_PRDC_args,
    add_sampler_FID_args,
    add_big_sample_args,
)
...
def add_logging_args(parser):
    ...
    parser.add_argument(
        "--save_individually", const=True, action="store_const", default=False
    )
...
def get_sampler_hparams():
    parser = argparse.ArgumentParser(
        "Parser for training discrete latent sampler models :)"
    )
...
def get_big_samples_hparams():
    parser = argparse.ArgumentParser(
        "Script for generating larger-than-training samples"
    )
